...
import { Chapter, CourseList } from "@/config/Schema";
...
const CourseStart = ({ params }) => {
  ...
  return (
    <>
    <Navbar />
    <div className="outfit-regular flex flex-col md:flex-row max-w-screen-xl mx-auto py-32 ">
      <div className="md:hidden w-full flex justify-end ">
        <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen} >
          ...
          <SheetContent className="w-full" side="right">
            <h2 className=" rounded-t-xl text-xl prata-regular">
      ...
      <div className="h-screen  rounded-xl border hidden md:block overflow-y-auto sticky top-0">
        <h2 className="p-3  rounded-t-xl text-xl prata-regula text-center bg-yellow-500 text-white">
          {course?.courseOutput?.course_name}
        </h2>
        <div>
          {course?.courseOutput?.chapters?.map((chapter, index) => (
